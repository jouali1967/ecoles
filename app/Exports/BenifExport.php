...
class BenifExport implements FromArray, WithHeadings, WithEvents
{

  protected $etudiants;
  protected $annee_scol;

  public function __construct($etudiants,$annee_scol)
  {
  ...
  public function array(): array
  {
    ...
    foreach ($this->etudiants as $etudiant) {
      $rows[] = [
        ...
        $etudiant->tel_pere ?? $etudiant->tel_mere ?? '',
        $etudiant->inscriptions->first()->classe?->abr_classe,
  ...
  {
    return [
      ['لائحة المستفيدات للموسم الدراسي '.$this->annee_scol],
      [],
      // Ordre inversé pour l'arabe : téléphone, classe, nom complet, code massar
      ['رقم هاتف ولي الامر', 'المستوى الدراسي', 'الاسم والنسب', 'رقم مسار'],
    ];
  }
    ...
    return [
      AfterSheet::class => function (AfterSheet $event) {
        ...
        foreach ($this->headings() as $heading) {
          $headerCount++;
          if ($heading === ['رقم هاتف ولي الامر', 'المستوى الدراسي', 'الاسم والنسب', 'رقم مسار']) {
            break;
          }
        }
