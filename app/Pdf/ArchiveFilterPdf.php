...
class ArchiveFilterPdf extends TCPDF
{
  protected $annee_scol;
  protected $sup_a;
  protected $inf_a;
  public function __construct($annee_scol = null,  $sup_a = null, $inf_a = null)
  {
  ...
  {
    if ($this->page == 1) {
      // Détermination du titre selon les cas
      if (!empty($this->sup_a) && !empty($this->inf_a)) {
        $titre = " الحاصلات على معدل اكبر من " . $this->sup_a . " و اصغر من " . $this->inf_a . " موسم " . $this->annee_scol;
      } elseif (!empty($this->sup_a) && empty($this->inf_a)) {
        $titre = " الحاصلات على معدل اكبر من " . $this->sup_a . " موسم " . $this->annee_scol;
      } elseif (empty($this->sup_a) && empty($this->inf_a)) {
        $titre = " الحاصلات على المعدل لجميع الطالبات موسم " . $this->annee_scol;
      } else {
        $titre = " الحاصلات على معدل اصغر من " . $this->inf_a . " موسم " . $this->annee_scol;
      }
