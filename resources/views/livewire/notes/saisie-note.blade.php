<div>
  <div class="mb-4">
    <h4 class="fw-bold text-primary d-flex align-items-center gap-2"
      ...
      @if($selectedEtudiant && $selectedEtudiant->inscriptions->isNotEmpty() &&
      $selectedEtudiant->inscriptions->first()->classe)
      <span class="badge bg-primary ms-2">Classe : {{ $selectedEtudiant->inscriptions->first()->classe->nom_classe }}
      </span>
      <span class="badge bg-secondary ms-2">Année : {{ $selectedEtudiant->inscriptions->first()->annee_scol }}</span>
      @endif
    </h4>
  </div>
  {{-- @dd($matieres) --}}
